...
class StationPage extends StatelessWidget {
  ...
  final String stn_id;
  final String basin_id;
  // ignore: non_constant_identifier_names
  StationPage({Key key, @required this.stn_id, this.basin_id})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: DefaultTabController(
        length: 4,
        child: MyDisplayClass(basin_id, stn_id),
      ),
    );
  }
}
...
Future<DataModelGet> getdata(String stn_id, int basin_id) async {
  ...
  if (response.statusCode == 200) {
    ...
    print('Response body: ${response.body}');
    // ignore: non_constant_identifier_names
    final DataModel = jsonDecode(response.body);
    print('Response JsonDecode: $DataModel');
    return DataModelGet.fromJson(DataModel);
  } else {
    throw Exception();
  }
}
